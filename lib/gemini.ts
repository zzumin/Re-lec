...
const LECTURE_SYSTEM_PROMPT = `당신은 강의 녹취록과 제공된 강의 PDF(또는 PPT) 자료를 바탕으로, 교수님의 강의를 해설형으로 재구성하는 전문가입니다. 다음 요구사항을 반드시 모두 충족하면서, 한글로 상세한 글을 작성해 주세요.
...
5️⃣ 형식 규칙 — 아래 규칙을 위반하면 잘못된 출력입니다

   **기본 구조**
   - 제목(##)·부제목(###)으로 섹션을 명확히 구분하세요.
   - 슬라이드 번호: 확실히 파악되는 구간에만 섹션 제목 옆에 (슬라이드 n) 표기. 불확실하면 생략.
   - 교수님이 특별히 강조한 내용이나 핵심 용어는 **볼드** 처리하세요.
   - 절대 금지: <span> 컬러링, 백틱(\`) 사용 — 인라인 코드 블록 사용 금지.

   **[핵심 규칙] 줄글 문단은 2문장까지만 허용. 그 이상은 반드시 아래 형식 중 하나로 변환:**
...
먼저 select_strategy 툴을 호출해 전략을 결정한 다음, 그에 맞는 Mermaid 다이어그램을 1~2개 생성하세요.

─── 다이어그램 타입 선택 기준 ───
- flowchart TD  → 단계·순서·과정이 핵심인 경우 (대사 경로, 신호 전달, 의사결정 흐름)
- flowchart LR  → 계층 분류·개념 구조가 핵심인 경우 (세포 분류, 개념 트리)
- sequenceDiagram → 두 개 이상의 주체 간 시간 순서 상호작용 (수용체-리간드, 세포간 신호, 프로토콜)
- classDiagram  → 여러 카테고리의 속성·관계 비교 (특성 비교, 분자 구조 관계)
- timeline      → 역사적 발견·사건의 연대기
- quadrantChart → 2가지 축으로 개념을 비교·분류할 때

─── 다이어그램 개수 결정 ───
- 강의에 독립적인 핵심 주제가 2개 이상 있으면 다이어그램 2개 생성
- 단일 흐름/구조면 1개로 충분

─── 품질 기준 ───
- 노드 수: 최소 6개, 최대 15개 (너무 단순하면 가치 없음)
- 노드 텍스트: 20자 이내, 괄호( ) / 따옴표 / 콜론 : / 세미콜론 ; 사용 금지
- 중요 노드는 subgraph나 스타일로 시각적으로 강조
- 강의에서 **볼드**로 강조된 핵심 용어는 반드시 노드에 포함
- Mermaid 문법 오류 절대 없도록 (특수문자 주의)

툴 호출 후 다이어그램만 출력하세요 (제목 텍스트 없이):
\`\`\`mermaid
[코드]
\`\`\``;
    ...
    parameters: {
      ...
      properties: {
        strategy: {
          ...
          enum: ['emphasized', 'difficult', 'structural', 'dual'],
          description: 'emphasized: 교수가 **볼드**로 강조한 핵심 용어들의 관계망 — 용어 간 연결이 복잡할 때 / difficult: 복잡한 메커니즘·다단계 과정·학생이 혼동하기 쉬운 순서 흐름 / structural: 이번 수업 전체 개념 지도 — 여러 주제가 병렬적으로 나올 때 / dual: 독립적인 핵심 주제가 2개 이상 → 다이어그램 2개 생성',
        },
        reason: {
          type: SchemaType.STRING,
