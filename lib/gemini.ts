...
출력 규칙:
- 반드시 아래 JSON 형식 그대로만 출력하세요. 코드블록, 마크다운, 설명 텍스트 없이 순수 JSON만 출력하세요.
- 단답형: 명확하고 간결한 정답이 있는 개념 확인 문제. 단순 암기보다 이해를 측정.
- OX: 참/거짓을 판단하는 문장. answer는 반드시 "O" 또는 "X".
- 빈칸채우기: 핵심 용어나 개념이 빠진 문장. 빈칸은 ___로 표시. hint에 해당 개념의 추가 맥락 제공.
- 서술형: 개념 설명, 메커니즘 서술, 비교 분석 등 심화 문제. answer에 모범답안 제공.

모든 문제는 explanation에 해설을 반드시 포함하세요 (왜 그 답이 정답인지, 관련 개념 연결).
...
export async function generateLectureNote(
  ...
  const lectureNote = lectureResult.response.text();
